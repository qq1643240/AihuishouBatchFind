...
#   ./scripts/build.sh rootless       # Dopamine / 越狱
#   ./scripts/build.sh roothide       # RELAXIN / roothide
#   ./scripts/build.sh rootful        # 老式 rootful 越狱
#
# 环境变量（被 GitHub Actions / 用户手动覆盖）:
#   THEOS_PACKAGE_SCHEME   同上参数优先级更高
#   THEOS                  Theos 工具链根目录

set -euo pipefail

# 决定 variant: 命令行参数 > THEOS_PACKAGE_SCHEME > 默认 rootless
